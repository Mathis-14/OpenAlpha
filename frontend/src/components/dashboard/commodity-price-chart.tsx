...
import { getCommodityPriceHistory } from "@/lib/api";
import type { CommodityInstrumentSlug, CommodityRange, PricePoint } from "@/types/api";

const RANGES: { label: string; value: CommodityRange }[] = [
  { label: "1D", value: "1d" },
  { label: "5D", value: "5d" },
  { label: "1M", value: "1mo" },
  { label: "3M", value: "3mo" },
...
export default function CommodityPriceChart({
  ...
  async function handleRangeChange(nextRange: CommodityRange) {
    ...
    setError(null);
    try {
      const history = await getCommodityPriceHistory(instrument, nextRange);
      setCache((prev) => ({ ...prev, [nextRange]: history }));
      setRange(nextRange);
    } catch {
      setError("Unable to refresh commodity price history right now.");
    } finally {
      setLoading(false);
    }
  }
